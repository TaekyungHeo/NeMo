...
from nemo.lightning.pytorch.callbacks.chakra import ChakraCallback  # Import ChakraCallback
...
@run.cli.factory
@run.autoconvert
def chakra_callbacks() -> run.Config[ChakraCallback]:
    return run.Config(
        ChakraCallback,
        start_step=5,
        end_step=10,
...
def base_pretrain_config(args) -> run.Partial:
    ...
    recipe = run.Partial(
        ...
        trainer=run.Config(
            ...
            callbacks=[
                run.Config(
                    ChakraCallback,  # Use ChakraCallback here
                    start_step=5,
                    end_step=6,
                    warmup_steps=1,
                    active_steps=5,
